...
export default function matrizACIMes() {
    ...
    useEffect(() => {
        const getMatrizACI = async () => {
            try {
                const response = await axios.get('/api/ac/matrizACI');
    ...
    const datosAcumulados = matrizACI.reduce((acc, { mes, activo_critico_institucional, cantidad }) => {
        // Creamos una clave única para cada combinación de mes y riesgo
        const clave = `${mapearNumeroMesANombre(mes)}-${activo_critico_institucional}`;
        // Si ya existe, sumamos a la cantidad existente
        if (acc[clave]) {
            acc[clave].v += cantidad;
        } else {
            // Si no existe, creamos una nueva entrada
            acc[clave] = { x: mapearNumeroMesANombre(mes), y: activo_critico_institucional, v: cantidad };
        }
        return acc;
    }, {});
